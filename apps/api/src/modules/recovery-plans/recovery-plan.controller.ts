...
export class RecoveryPlanController {
  constructor(private readonly service: RecoveryPlanService) {}

  getOne = async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
      const result = await this.service.getRecoveryPlan(req.params.id, req.user!.id, req.user!.role);
  ...
  getAll = async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
      ...
      const { data, meta } = await this.service.getRecoveryPlans(req.user!.id, req.user!.role, query);
